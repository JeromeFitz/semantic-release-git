 ...
 * @customization
 */
const getCommitMessage = ({ env, message, nextRelease, repositoryUrl }) => {
  return (
    message
      // @todo Fallback if not monorepo
      .replace(/\{PACKAGE_NAME\}/g, env.LERNA_PACKAGE_NAME)
...
module.exports = async (pluginConfig, context) => {
  ...
  if (filesToCommit.length > 0) {
    ...
    await commit(
      message
        ? getCommitMessage({ env, message, nextRelease, repositoryUrl })
